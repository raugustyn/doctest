...
{
    //return "http://localhost:63342/TB04CUZK001_CartoModel/workflow/web/testdata/feature_" + index.toString() + ".wkt";
    return "http://localhost/TB04CUZK001/CartoModel/workflow/rest/getfeature.py?index=" + index.toString()
}
...
{
    if (index < 0) index = 0;
    if (index >= FeaturesCount) index = FeaturesCount - 1;
    ...
    $.get(getWKTUrl(index), function (data) {
        ...
        addFeature(originsSource, dataJSON.optimalCircle.origin);
        addFeature(circlesSource, dataJSON.optimalCircle.wkt);

        var feature = addFeature(reliefLinesSource, dataJSON.inputFeature.wkt);
